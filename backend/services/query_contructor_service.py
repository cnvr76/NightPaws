...
class QueryConstructor:
    ...
    def construct_queries(self, application: Application) -> Tuple[Optional[str], ...]:
        ...
        q2 = self._construct_sender_query(application)
        q3 = self._construct_company_query(application)
        return (q1, q2, q3)
    ...
    def _construct_wide_query(self, application: Application) -> Optional[str]:
        ...
        # date_string: str = last_date.strftime('%Y/%m/%d')
        # date_query: str = f"after:{date_string}"
    ...
    def _construct_sender_query(self, application: Application) -> Optional[str]:   
        # from:hyperia after:{date} in:inbox - first company name
        pass


    def _construct_company_query(self, application: Application) -> Optional[str]:
        # hyperia {filters} in:inbox - full company name, not just the first word
        pass
